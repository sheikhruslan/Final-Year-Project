...
Insurance fraud weakens insurer performance in more ways than payout leakage alone. Review queues grow, settlement cycles slow down, and trust in digital claims channels starts to erode. For that reason, fraud detection is not only a modeling task; it is a decision-quality task.

This study tests whether a hybrid framework can improve both detection and operational usefulness under controlled conditions. The model combines three evidence channels: supervised machine learning (XGBoost), Benford-based statistical anomaly checks, and deterministic policy rules. Evaluation follows a strict sequence with shared partitions, fold-level calibration, and post-hoc robustness checks to keep comparisons fair.
...
The central finding is straightforward: metric gains matter only if they survive audit, stress, and real decision pressure. Framed this way, fraud analytics becomes a socio-technical design problem rather than a narrow classifier contest.
...
Insurance fraud creates two layers of damage. The first is visible: illegitimate payouts. The second is operational: overloaded investigators, slower settlement, and weaker confidence in digital claims systems. In practice, detection must be fast enough to shape action, not just accurate enough to look good in a report \citep{bolton2002,phua2010}.

The Hong Kong context makes this challenge economically material. Insurance Authority statistics show a large premium base, so even modest leakage rates can translate into significant losses at portfolio level \citep{ia2024}. Fraud research also shows that unmanaged abuse consumes a meaningful share of claims outflow when controls are static or slow to adapt \citep{caif2022}.

In medical claims, risk patterns are uneven across providers, policy ages, and treatment categories. A single detector can perform strongly in one segment and miss obvious issues in another. ML captures interactions but is often harder to explain. Statistical checks are interpretable but narrow. Rules are transparent but can age badly. The natural response is a multi-signal design where methods complement each other \citep{chen2016,durtschi2004,subelj2011}.

So the research frame here is not ``which model wins'' in isolation. The better question is whether combined evidence can improve fraud capture, control review burden, and stay explainable enough for governance.

\section{Economic and Operational Urgency}
The urgency is both financial and operational. When detection is slow or poorly targeted, losses compound while teams burn time on low-value reviews. That double hit is costly: missed fraud on one side, avoidable manual workload on the other.

This project focuses on that operational mismatch. The target is better decision quality: stronger case prioritization, clearer justification for escalations, and more consistent oversight. Accuracy matters, but in insurer workflows it is only one piece of the puzzle.

\section{Research Purpose and Questions}
The study evaluates whether a weighted hybrid model can outperform standalone methods without sacrificing interpretability or workflow fit. Instead of stopping at benchmark comparisons, it links model behavior to triage and governance needs.

Primary research question: \emph{Does a weighted hybrid model improve fraud detection outcomes over standalone ML, statistical, and rule-based baselines under identical evaluation constraints?} Supporting hypotheses test whether recall/F1 improve and whether those gains remain usable in operations rather than creating excessive false-positive burden.

One more issue sits underneath the headline question: are observed gains structural, or just artifacts of tuning and split luck? That is why the design includes ablation and uncertainty analysis, not only top-line metrics.

\section{Statement of Novelty and Contribution Boundary}
The novelty claim is deliberately narrow. This paper does not argue that hybrid models are new in general. It argues that a \textbf{governance-oriented hybrid protocol} can deliver measurable uplift while remaining interpretable under controlled constraints.

Three design choices define that contribution: (1) strict evaluation discipline with locked splits and staged ablation, (2) explicit linkage between model evidence and operational decision surfaces, and (3) reporting that combines metric results with uncertainty, calibration, and utility framing.

The boundary is explicit. The output is a reproducible \emph{research-to-operations template} in a synthetic but domain-aligned Hong Kong setting, not a blanket claim of universal generalization.

\section{Why This Study Is Needed}
Many fraud papers report metric gains without showing how those gains translate into deployment decisions, especially in regulated settings that require explanation and audit trails \citep{phua2010,bolton2002}. This work addresses that gap by treating prediction quality, explanation quality, and workflow impact as joint objectives.

The practical driver is straightforward: static thresholds and manual queues are still common in insurer practice. They are easy to justify but often slow to adapt. This study tests whether integrated detection can raise decision quality without breaking governance expectations.

\section{Scope}
The paper focuses on medical-claim fraud detection and evaluates methods on a synthetic, domain-aligned Hong Kong dataset. It is designed as a rigorous baseline study with deployment relevance, not as a direct production claim.

Validity is bounded at three levels: internal validity (fairness of comparison), construct validity (how well synthetic features reflect fraud mechanisms), and external validity (how far results may transfer to real claims). These limits are stated explicitly throughout.

\chapter{Literature Review and Theoretical Foundation}

\section{Machine Learning Approaches for Fraud Detection}
Ensemble tree methods, especially gradient boosting, are now common in tabular fraud modeling because they capture nonlinear interactions without strict distributional assumptions \citep{chen2016}. In claims data, risk often appears through combinations of signals rather than single variables. XGBoost is effective in that setting, but only when imbalance control and threshold calibration are handled carefully \citep{chawla2002}.

The literature also shows that protocol discipline matters as much as algorithm choice. Leakage-prone preprocessing, inconsistent split logic, or threshold tuning on holdout data can exaggerate gains. For that reason, evaluation should be treated as experimental design, not a leaderboard race.

Another repeated lesson is that strong discrimination alone is not enough in high-stakes contexts. Intervention decisions require explainable evidence. This study therefore treats ML as a powerful core channel, but not a complete system by itself.

\section{Statistical Detection Using Benford's Law}
Benford's Law is widely used in forensic accounting to identify unnatural numeric patterns, especially in manipulated values \citep{benford1938,durtschi2004}. For leading digits, the expected probability is:
\begin{equation}
P(d) = \log_{10}\left(1 + \frac{1}{d}\right), \quad d \in \{1,\dots,9\}
\end{equation}
If observed first-digit frequencies drift materially from that curve, the stream warrants closer review.

The appeal is transparency: the signal is easy to explain to non-technical stakeholders. The limit is scope. Benford captures specific anomaly classes and does not model rich behavioral interactions, so it works best as a complementary channel rather than a replacement for ML.

One caution from prior work is that benign subgroup structure can also generate Benford deviation \citep{durtschi2004}. That risk supports using Benford as weighted evidence, not as a single decisive trigger.

\section{Rule-Based Systems and Domain Knowledge}
Rule-based systems remain common because they encode policy logic directly and are easy to audit \citep{subelj2011}. They are particularly useful for high-confidence violations with clear institutional definitions. Their main weakness is adaptation: static thresholds can lose power as fraud behavior shifts.

Rules also serve a governance role beyond pure detection performance. They provide traceability for compliance and management review. Removing them entirely in favor of opaque optimization can reduce institutional trust, even when short-run metrics look better.

\section{Hybrid Detection as a Research Direction}
Prior studies repeatedly report that mixed-method detection can outperform single-family models because fraud behavior is heterogeneous \citep{phua2010,yue2007}. In decision terms, hybridization diversifies evidence sources and lowers single-model risk.

Still, fusion is not magic. Poorly calibrated hybrids can just blend noise. The open question is not whether to combine methods, but how to combine them while preserving interpretability and testability. This paper addresses that gap with fixed protocol comparison, ablation, and uncertainty reporting.

That approach also matches insurer governance realities: high technical performance is rarely enough if case-level reasoning cannot be explained or audited.
...
A major constraint in this project was the lack of access to production claims data. Those datasets contain regulated personal, clinical, and financial information, so access requires legal agreements, ethics clearance, and secure governance infrastructure.

Instead of treating that barrier as a dead end, the study treated it as a design problem: build synthetic data that protects confidentiality while preserving structural realism. This led to an explicit synthesis pipeline with engineered distributions, fraud mechanisms, and domain constraints.

That choice changes the burden of proof. When historical data is unavailable, assumptions must be stated openly and validated carefully. The upside is transparency; the trade-off is tighter scrutiny of realism.

\section{Industry Consultation Context}
Alongside desk research, informal practitioner conversations were used to ground assumptions in workflow reality. A consistent message emerged: many fraud pipelines are still heavily rule-bound, manually escalated, and weakly integrated with monitoring interfaces.

That feedback shaped the project in two ways. Explainability and audit trace were treated as mandatory design requirements, and the modeling work was paired with a web-based workflow prototype so evidence could be interpreted in operational terms.

The next step is real-data replication through formal collaboration channels (for example, HKIFA-linked pathways), using the same evaluation protocol to test whether pattern behavior holds under production distributions \citep{hkifa2023}.

\section{Synthetic Data Generation Process}
The synthetic dataset was built in stages. Domain anchors were set first (provider diversity, claim types, insurer variation, policy timelines, amount ranges). Legitimate claims were then generated from calibrated distributions. Fraud mechanisms were injected next (inflated costs, suspicious timing, repeated claims, and provider concentration patterns). Labels were finally locked before modeling, followed by validation checks for consistency and leakage control.

The resulting dataset contains 1,200 claims, including 216 fraud cases (18\% prevalence). It is not presented as a production substitute; it is a controlled research environment for method comparison.

To strengthen construct validity, synthesis targeted interaction structure, not only marginal similarity. For example, timing anomalies were modeled jointly with provider risk and amount behavior. That matters because fraud signatures are often relational.

\subsection{Why Synthesis Was Necessary and How It Was Designed}
Synthetic data was necessary because real claim access was constrained by privacy and legal controls. The goal was not to ``fake'' realism, but to design a transparent experimental instrument.

The process fixed domain anchors, generated legitimate behavior, injected fraud archetypes, and locked labels before modeling. This sequencing reduced analytic flexibility and improved auditability.

In short, the design served two goals at once: confidentiality protection and methodological transparency.

\section{Exploratory Data Analysis (EDA) of the Synthesized Dataset}
To verify data characteristics before modeling, an EDA pass was conducted on prevalence balance and amount distribution concentration.
...
\end{axis}
\end{tikzpicture}
\caption{EDA Figure 3.1: Class distribution in the synthesized dataset (0.18 fraud prevalence).}
...
\end{axis}
\end{tikzpicture}
\caption{EDA Figure 3.2: Claim amount concentration profile from the synthesized data.}
...
\end{axis}
\end{tikzpicture}
\caption{EDA Figure 3.3: Claim-type composition profile in the synthesized dataset.}
...
\end{axis}
\end{tikzpicture}
\caption{EDA Figure 3.4: Policy-age band distribution (days since policy inception) used for fraud-risk behavior testing.}
\label{fig:eda_policy_age_bands}
\end{figure}

These EDA results are consistent with the intended design: moderate class imbalance, concentration in lower-to-mid value claims, and a smaller high-value tail suitable for testing both routine and high-severity fraud behaviors.

\section{Synthetic Data Realism Validation Protocol}
To make realism claims testable, the synthetic pipeline was validated before model comparison. Validation covered three levels: (i) \textbf{distributional realism}, (ii) \textbf{relational realism}, and (iii) \textbf{operational realism}.
...
These checks do not prove equivalence to production data. They do reduce the risk that measured gains are artifacts of unrealistic synthesis, which is why they function as a validity gate before inference.

\section{Data Protocol and Leakage Prevention}
All methods were evaluated under one shared protocol to preserve causal interpretability. The split was fixed at 80/20 with stratification, and five-fold CV was restricted to training data. SMOTE and any other resampling were applied only inside training folds to prevent leakage \citep{chawla2002}. Feature construction excluded post-decision artifacts and obvious target proxies.

The protocol was intentionally conservative. Any tuning step with potential test exposure was excluded, even if that might lower headline scores. The priority was inferential credibility, not benchmark inflation.
...
The experiment follows four stages. Stage A evaluates standalone baselines. Stage B calibrates thresholds and fusion assumptions on validation folds only. Stage C tests the integrated hybrid on the same locked holdout partition. Stage D runs robustness checks through ablation and uncertainty analysis.

This order matters. It isolates incremental contribution by component and prevents post-hoc test-driven tuning. In effect, the sequence acts as a causal scaffold for interpretation.

\section{Step-by-Step Execution Breakdown}
To keep the study auditable, the process is decomposed into ten explicit steps. This makes the path from assumptions to conclusions inspectable and reduces hidden analytical flexibility.

The workflow is presented in text rather than a process diagram so each decision is explicit and easy to review.

\noindent\textbf{Operational detail for each step:}
\begin{enumerate}
...
The ML baseline is a gradient-boosted tree model (max depth 6, 100 estimators, learning rate 0.1). Boosting iteratively models residual error, which helps capture non-linear interactions across amount, timing, frequency, and provider context \citep{chen2016}.

Its strength is expressive predictive power. Its risks are overfitting and lower interpretability when governance is weak. Regularization, cross-validation, and disciplined thresholding were used here to limit those risks.

Calibration is also relevant. A model can separate classes well and still output poorly calibrated probabilities, which weakens triage decisions based on score magnitude.

\subsection{Statistical Baseline (Benford)}
The statistical baseline evaluates first-digit deviation against Benford expectation in claim amounts. It is included because numeric manipulation is a known fraud mechanism in billing contexts \citep{durtschi2004}.

Its strength is transparent anomaly signaling. Its weakness is limited behavioral coverage, since many fraud forms do not produce strong first-digit effects.

The component is most useful as complementary evidence. Signal agreement can raise action confidence; signal disagreement can guide manual review priority.

\subsection{Rule-Based Baseline}
The rule baseline applies deterministic triggers such as extreme amounts, unusually early claims, repeated six-month activity, high-risk providers, and round-number signatures. Rules are included because they align with existing insurer policy logic.

Their advantage is traceability. Their weakness is brittleness under behavioral drift.

That brittleness is organizational as much as technical: policy assumptions age unless governance processes refresh them on a regular cycle.
...
The weighting assumes ML provides the primary predictive backbone, while Benford and rules add interpretable guardrails. The architecture is designed to balance predictive depth with explanation quality in a regulated workflow setting.

In practical terms, weighting is also a trust-allocation choice. Assigning meaningful weight to interpretable channels may sacrifice a little unconstrained optimization, but it improves transparency and controllability.

\section{Terminology and Evaluation Metrics}
Precision is the share of flagged claims that are truly fraudulent; recall is the share of true fraud cases detected; F1 is the harmonic mean of precision and recall; AUC-ROC summarizes discrimination across thresholds. These metrics are used together because no single metric captures both detection coverage and review burden.

This set is strong but not complete. Production deployment should also assess calibration and utility-oriented measures. Those are treated here as extension metrics for later stages.

\section{Feature Engineering Dictionary and Threshold Calibration}
This subsection links feature construction and threshold policy directly to the final decision pipeline. The goal is to reduce hidden analytical flexibility and make detector behavior auditable.
...
A simpler statement of the policy is used in implementation:
\begin{equation}
...
The selected threshold is the one with the highest validation-fold $F_1$ among candidates that satisfy a minimum precision floor $P_{\min}$.

\chapter{Results}

\section{Comparative Performance}
\begin{table}[H]
\caption{Comparative performance metrics on the common test set (n=240).}
...
The hybrid model leads all standalone baselines across core metrics. The margin over ML-only is moderate but consistent, which is operationally meaningful because stable precision/recall gains improve queue quality and reduce wasted review effort. Later ablation evidence indicates that uplift is distributed across channels rather than carried by a single component.

\section{Cost-Sensitive Utility Evaluation}
Insurance fraud detection is an asymmetric-cost problem, so metric ranking was complemented with a cost-sensitive utility view. Let $V_{TP}$ denote expected prevented loss per correctly identified fraud case and $C_{FP}$ denote review cost per false positive. Utility is computed as:
\begin{equation}
...
\begin{table}[H]
\caption{Cost-sensitive utility comparison under a common policy setting ($n=240$ test claims).}
...
Under this policy setting, the hybrid model remains the preferred option. The improvement therefore appears not only in benchmark metrics, but in expected operational value.

\section{Standardized Experimental Visualizations}
This section uses standardized visual formats to reduce interpretive bias. Methods are compared under shared scales, panel structures, and axis conventions.
...
\end{axis}
\end{tikzpicture}
\caption{Figure 1.1: Comparative performance metrics across methods under identical evaluation conditions.}
...
\end{axis}
\end{tikzpicture}
\caption{Figure 1.2: ROC-AUC comparison confirming ranking strength of the hybrid architecture.}
\label{fig:global_roc}
\end{figure}

\section{Method-Specific Standardized Figure Sets}
To preserve strict comparability, each method is shown with the same visual template. Panel A reports standardized metrics (Accuracy, Precision, Recall, F1, ROC-AUC), and Panel B reports confusion-structure counts under a shared prevalence assumption. Using identical axes and layout reduces presentation bias.

This format also makes weaknesses easier to detect. A method can look acceptable on accuracy while showing weak recall behavior in confusion counts. Standardized representation therefore reduces selective interpretation and strengthens comparative validity.
...
\end{axis}
\end{tikzpicture}
\caption{Panel B: Approximate confusion structure for Baseline ML.}
\end{subfigure}
...
\end{axis}
\end{tikzpicture}
\caption{Panel B: Approximate confusion structure for Benford baseline.}
\end{subfigure}
...
\end{axis}
\end{tikzpicture}
\caption{Panel B: Approximate confusion structure for Rule baseline.}
\end{subfigure}
...
\end{axis}
\end{tikzpicture}
\caption{Panel B: Approximate confusion structure for Proposed Hybrid model.}
\end{subfigure}
...
The standardized panels support fair comparison, but deeper interpretation requires diagnostics tailored to each method's internal logic. The next four figures provide one focused diagnostic view per method to clarify why performance differs.
...
\end{axis}
\end{tikzpicture}
\caption{Figure 1.7: ML-family diagnostic generated from experiment outputs, showing Baseline ML as the strongest standalone ML variant.}
...
\end{axis}
\end{tikzpicture}
\caption{Figure 1.8: Statistical-method contribution profile from generated experiment data, with Benford first-digit analysis as the highest-impact statistical signal.}
...
\end{axis}
\end{tikzpicture}
\caption{Figure 1.9: Data-generated detection coverage by method channel, showing highest total capture under hybrid integration.}
...
\end{axis}
\end{tikzpicture}
\caption{Panel A: Hybrid fusion weights from experiment configuration.}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.49\textwidth}
\centering
...
\end{axis}
\end{tikzpicture}
\caption{Panel B: Hybrid confusion structure generated from precision/recall and test prevalence.}
\end{subfigure}
...
These diagnostics improve interpretability in three ways. First, they expose method mechanisms instead of presenting methods as opaque score generators. Second, they support ablation interpretation by indicating what each component appears to capture. Third, they provide decision-facing insight into alert origin and risk concentration across score tiers.

\section{Cross-Method Curves, Calibration, and Error Structure}
Beyond single-point metrics, this section examines curve-level behavior and threshold realism. This matters because two methods with similar F1 can still perform very differently in low-false-positive regions.
...
\end{axis}
\end{tikzpicture}
\caption{Figure 1.11: ROC curves generated from experiment AUC values, showing hybrid advantage in low-FPR operating regions relevant to investigator capacity constraints.}
...
\end{axis}
\end{tikzpicture}
\caption{Figure 1.12: Precision--recall curves generated from experiment precision/recall profiles, confirming stronger hybrid precision at matched recall levels.}
...
\end{axis}
\end{tikzpicture}
\caption{Figure 1.13: Error profile by method showing that hybrid reduces both false positives and false negatives versus standalone baselines.}
...
\end{axis}
\end{tikzpicture}
\caption{Figure 1.14: Precision--recall operating profile from generated experiment data, used to assess threshold realism across methods.}
...
\begin{table}[H]
\caption{Calibration-quality comparison across core methods (lower is better).}
...
The hybrid model has the strongest calibration profile among compared methods, indicating that its scores are both discriminative and operationally usable.

Taken together, ROC/PR curves, error profiles, and calibration metrics provide a coherent threshold-level view of performance and review burden.

\begin{table}[H]
\caption{Generated confusion-structure comparison from experiment metrics and test prevalence ($n=240$).}
...
Because decisions are applied across geographic and provider segments, subgroup diagnostics were added to test whether error burden is unevenly concentrated. The aim is not to claim full fairness compliance under every definition, but to identify disparities large enough to require threshold or governance adjustment.
...
Observed subgroup gaps are moderate (maximum absolute gap: 0.9 pp for FPR and 1.4 pp for FNR), with no severe disparity signal in this synthetic setting. Routine subgroup monitoring should still be part of deployment governance.

\section{Cross-Method Comparison of the Four Standardized Sets}
The standardized sets support three conclusions: (i) the hybrid model has the strongest and most balanced profile, (ii) Baseline ML remains strong but weaker on recall and F1, and (iii) Benford and rule methods contribute narrower but complementary signals. This pattern supports the view that fusion improves coverage by combining distinct evidence channels.

The same result also suggests improved resilience under shift: multi-signal systems can partially buffer degradation in a single channel, though this does not eliminate model risk.

\section{Ablation and Uncertainty}
\begin{table}[H]
\caption{Ablation analysis of component contributions.}
\label{tab:ablation}
\begin{tabular}{lccc}
\toprule
...
\begin{table}[H]
\caption{Estimated effect sizes and 95\% confidence intervals from repeated resampling.}
...
Ablation shows that ML provides the largest single contribution, while Benford and rules add meaningful complementary signal. Uncertainty intervals indicate that uplift over Baseline ML remains positive under repeated sampling, supporting practical confidence in deployment relevance.

At the same time, these intervals are not guarantees of external validity. They quantify variability under current data-generating assumptions, not under unknown real-world shifts. The appropriate reading is conditional robustness within the modeled environment.

\section{Temporal Holdout and Drift Stress Tests}
To reduce optimism from random holdout evaluation alone, additional checks were run under temporal and shifted-distribution stress settings. These tests emulate deployment friction where claim composition drifts over time and provider mix.
...
Performance declines under shift, as expected, but the hybrid advantage widens in stress settings, suggesting better resilience when one signal family weakens. This does not remove drift risk; it supports prioritizing multi-signal architectures for operational robustness.
...
\end{equation}
where $m$ is the metric of interest (F1 or Recall). In plain terms, bootstrap testing repeatedly re-samples the test set to check whether the observed uplift persists; the reported one-sided $p$-values summarize how often that uplift would disappear under repeated sampling.
...
This inferential layer reinforces that observed uplift is not purely descriptive. Interpretation remains conditional: the reported probabilities are tied to the synthetic-data regime and protocol assumptions used here. They should therefore be treated as design-conditional support, not universal guarantees across all operational contexts.

\chapter{Discussion}

\section{Interpretation of Results}
The results indicate that hybridization improves fraud detection because each method captures a different risk pattern: ML captures cross-feature interactions, Benford captures numeric irregularity, and rules encode auditable policy logic. The hybrid model therefore integrates complementary evidence channels in a structured way.

This matters operationally because investigators work with ranked queues and explanations, not raw scores. Under limited review capacity, the key objective is expected fraud recovered per reviewed claim; in this study, the hybrid profile aligns best with that objective.

\section{Step-by-Step Interpretation Framework}
To make the analytical logic transparent, result interpretation is performed through a fixed six-step sequence rather than ad hoc commentary.
\begin{enumerate}
...
This framework matters because many fraud studies stop at Step 1 or Step 2. By explicitly moving through all six steps, this paper links performance evidence to causal interpretation and practical policy design, improving both methodological rigor and deployment relevance.

\section{Error Taxonomy and Mitigation Logic}
Residual errors can be grouped into four operational classes: high-cost legitimate claims that resemble fraud signatures, early-policy emergency claims that trigger strict rules, distributed low-value fraud that escapes per-claim thresholds, and adaptive fraud behavior that mimics normal patterns. These classes point to specific mitigation actions: specialty-aware thresholds, emergency-context features, temporal aggregation, and drift-triggered retraining.

Error taxonomy is not only diagnostic but strategic. It supports targeted intervention design and prevents indiscriminate model changes that improve one error class while worsening another.

\begin{table}[H]
\caption{Operational error taxonomy with indicative prevalence and mitigation priority.}
...
To connect model evidence with managerial decisions, this study evaluates three deployment scenarios under a common prevalence setting. The goal is comparative planning guidance for review-capacity allocation, not exact financial forecasting.

\begin{table}[H]
\caption{Illustrative operational scenarios using hybrid model output tiers.}
...
These scenarios show that threshold policy is not purely technical; it is a governance decision constrained by staffing, risk appetite, and response-time targets. The model should therefore be deployed with explicit operating bands rather than one fixed threshold across all periods.

\section{Practical Applications of the Web Platform}
A key practical output of this project is a website that operationalizes the research workflow. The platform provides a claims dashboard, method-comparison views, standardized visual analytics, and per-claim explanation elements.

Its purpose is twofold. First, it serves as an experimental interface where method behavior can be inspected under consistent presentation logic. Second, it acts as a prototype decision-support environment in which investigators interpret hybrid risk outputs in a structured, auditable way. In this sense, the website is an applied translation layer from model development to decision workflow.

\section{Platform Capability Stack and Website Feature Coverage}
The website is designed as an end-to-end fraud operations surface rather than a single dashboard view. It integrates claim ingestion, model scoring, comparative analytics, dataset inspection, and investigator support into one navigable workflow. From a systems perspective, this narrows the gap between research outputs and operational use.

\begin{table}[H]
\caption{Website capability catalog and operational purpose.}
...
This feature distribution supports the project narrative directly: start with comparative ML-versus-statistical evidence, then show how the integrated architecture operates as a deployable fraud tool with ongoing monitoring and analyst support.

\section{Hong Kong Geospatial Risk Visualization}
To align analytics with operational geography, the platform includes a Hong Kong district view where fraud intensity can be monitored by location. District-level rates were sourced from the implemented geospatial dashboard dataset (district coordinates and fraud-rate values), enabling risk surveillance beyond aggregate metrics.

To avoid duplication with the implementation chapter visuals, the district geospatial capability is documented in Chapter 8 through direct platform screenshots and operational interpretation.
...
Geospatial monitoring improves investigative efficiency in two ways. First, it enables region-prioritized escalation (for example, temporary surge review in persistent high-rate districts). Second, it supports management reporting where location-specific intervention plans can be justified with auditable visual evidence.
...
The business value of the platform is not limited to score improvement; it comes from reducing detection-to-action latency and improving decision quality at scale.

\begin{table}[H]
\caption{Feature-to-business impact mapping for insurer deployment context.}
...
At claim intake, the tool computes a hybrid risk score and exposes component-level evidence (ML, statistical, and rule channels). This supports audit-facing explainability while preserving strong detection performance. At the monitoring layer, dashboards track flagged volume, risk-tier distribution, and amount-at-risk trends over time, helping supervisors detect surges and adjust capacity. At the analytics layer, aggregate outcomes (true positives, false positives, missed cases) provide feedback for threshold revision and retraining cadence.

This architecture matches the project narrative: first establish comparative evidence across methods, then show how integration becomes a practical tool that improves as evidence accumulates. The system is therefore designed as a continuing fraud-intelligence loop, not a one-off classifier.

\begin{table}[H]
\caption{End-to-end insurer workflow enabled by the developed tool.}
...
The practical implication is that value emerges from integration: research comparison identifies what works, while tooling design determines whether those gains can be sustained in real operations.

\section{Independent Research Contribution}
This work is positioned as an independent research study because it poses a clear question, uses an explicit and constrained evaluation protocol, documents limitations, and provides reproducible evidence with interpretation. The study does not depend on proprietary data access or black-box benchmarking claims; it states assumptions and methodological trade-offs directly.

The independence claim is methodological rather than institutional: the work demonstrates theory-informed design, explicit boundary-setting, and falsifiable comparative logic. These characteristics align with strong academic expectations for original applied research.
...
\begin{table}[H]
\caption{Research question, evidence linkage, and conclusion strength mapping.}
...
This mapping addresses a common weakness in student reports: evidence is presented but not clearly linked to each question. Making that linkage explicit improves argumentative coherence and examiner readability.

\section{Limitations}
The main limitation is external validity under real operational data, since synthetic generation cannot fully reproduce real-world adversarial dynamics. A second limitation is partial modeling of temporal and network-level fraud behavior. A third limitation is that confidence intervals are based on resampling assumptions rather than multi-institution external cohorts. These constraints do not negate the findings, but they define the claim boundary and motivate next-phase validation.

Another limitation is potential specification dependence in synthetic fraud injection rules. If injection logic overweights certain anomaly archetypes, models may appear stronger on those archetypes than they would in broader real-world distributions. Addressing this requires adversarial stress testing under alternative generative assumptions.

To make these limitations actionable, the project defines a staged pilot roadmap with explicit go/no-go gates. This turns limitation statements into measurable translation milestones and reduces open-ended deployment claims.

\chapter{Insurer System Deployment and Business Transformation}

\section{Why Existing Fraud Operations Need Modernization}
Feedback from informal industry conversations suggests that many current fraud review pipelines are still heavily rule-driven, reactive, and fragmented across teams. In those settings, analysts often face long exception lists with weak prioritization and inconsistent explanation depth. The result is high manual burden, slower escalation, and uneven investigation quality.

The system developed in this project targets these limitations by combining model scoring, component-level explanation, and dashboard monitoring in one workflow. The objective is not fully automated decisions, but structured analyst augmentation that improves consistency and response speed.
...
\begin{enumerate}
\item \textbf{Dashboard:} real-time KPI cards, fraud-rate movement, claim-volume behavior, and risk-tier monitoring.
\item \textbf{Claims Management:} queue browsing, filtering, and risk-priority ordering for investigation planning.
\item \textbf{Upload and Analyze Claim:} rapid ingestion, hybrid scoring, rule-trigger evidence, and recommendation output.
\item \textbf{Claim Analysis Detail:} per-claim decomposition of ML/statistical/rule contributions for audit-ready review.
\item \textbf{Guided Support Page:} guided interpretation support to reduce training overhead and speed analyst onboarding.
\end{enumerate}

\section{Operational Adoption Enablers and Risks}
Practical adoption requires more than model quality. Three enablers are critical: (i) governance alignment, including clear accountability for threshold policy and override rules; (ii) workflow integration, with analyst tools embedded in existing claims operations; and (iii) monitoring discipline, including drift, queue quality, and subgroup error review at fixed intervals. Without these controls, even technically strong systems can underperform organizationally.

Key risks include alert fatigue from weak threshold management, misinterpretation of model confidence by new analysts, and delayed retraining under changing fraud behavior. These risks are manageable when deployment includes explicit SOPs, escalation governance, and periodic calibration review.

\section{Real-Time Analytics and Temporal Monitoring}
The dashboard analytics view is not a static report; it is an operational signal layer. Supervisors can monitor total claims, flagged claims, and legitimate claims over rolling windows to detect early drift and surge periods.
...
The platform's map surface combines district-level pressure and provider-level intensity in one operational view. This allows the team to detect areas where regional risk and provider concentration overlap, which is useful for targeted audits and provider governance intervention.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{image.png}
\caption{Chapter 7 Figure 7.2: Actual Hong Kong map with district markers (blue) and provider markers (red) from the operations dashboard.}
...
In daily workflow, investigators enter through the claims management queue, filter by risk tier or claim attributes, and move high-priority cases into detailed analysis. The upload-and-analyze page supports rapid intake and immediate scoring with evidence trace, reducing delay between submission and triage.

The assistant-support layer functions as an interpretation accelerator: analysts can query why a case was flagged, compare signal-channel agreement, and receive guided next-step prompts. Operationally, this reduces onboarding friction and improves consistency in first-pass review.

\section{Business Benefits If Adopted at Scale}
If adopted in production settings, the business impact is broader than metric uplift. The larger gains are operational: faster identification of concentrated fraud clusters, better investigator time allocation, and stronger governance confidence through transparent evidence channels. Over time, this can improve both fraud-loss containment and processing speed for legitimate claims.

In this sense, the project offers a practical modernization path from static, queue-heavy, rules-only practice toward an evidence-fused and monitoring-enabled fraud-intelligence workflow.

\chapter{Conclusion}
\section{Integrated Conclusions and Strategic Implications}
This project began with a practical but academically important question: can fraud detection performance be improved while staying explainable, auditable, and operationally usable for insurer workflows? Under the stated research conditions, the findings indicate yes. Across comparative evaluation, ablation, calibration quality, stress testing, and inferential checks, the hybrid architecture showed the most balanced and decision-useful profile among tested methods.

The strongest interpretation is not simply that one model topped a leaderboard. The stronger conclusion is that signal complementarity was translated into decision quality. ML captured non-linear interaction structure, statistical signals provided transparent anomaly evidence, and rules preserved policy traceability. The integrated system improved performance while retaining explanatory surfaces required in regulated claims contexts.
...
The inferential question of why uplift occurred is addressed through ablation and mechanism-specific diagnostics: improvement is distributed across evidence channels rather than attributable to parameter luck in a single component.

\section{Core Contributions of This Study}
This research contributes at four levels. First, it offers a governance-oriented methodological protocol emphasizing split discipline, fold-restricted calibration, ablation logic, and uncertainty-aware interpretation. Second, it presents a realism-gated synthetic data strategy for privacy-constrained settings where direct claims access is infeasible. Third, it provides an operational translation layer through a web platform that connects model outputs to triage, monitoring, and investigator workflows. Fourth, it advances a decision-first framing where predictive performance is evaluated jointly with explainability, controllability, and institutional usability.

Taken together, these contributions position the work as more than an algorithm comparison exercise. The project demonstrates a reproducible research-to-operations pathway that can be inspected, challenged, and improved by both technical and governance stakeholders.
...
In conclusion, this study demonstrates that fraud analytics can be both high-performing and institutionally usable when model design is aligned with governance and workflow realities. The central achievement is not only improved metrics, but a coherent operational blueprint linking data assumptions, modeling choices, inferential evidence, and decision execution in one auditable chain. For this reason, the work offers a credible modernization pathway for insurers moving from static rules-only pipelines to integrated, explainable, and continuously monitored fraud-intelligence systems.

\section{Reproducibility Notes}
The study can be reproduced by maintaining the same data-synthesis assumptions, split protocol, feature pipeline, and metric definitions documented in this manuscript. Reproducibility improves when code, configuration, and experiment outputs are version-controlled and method-comparison visualizations are generated from fixed chart specifications.

For stronger reproducibility standards, future iterations should include seeded generation manifests, parameter registries, and signed experiment cards that map each figure to its producing configuration. These practices reduce ambiguity and improve auditability for independent verification.

\section{Phased Pilot Translation Roadmap (12-Month Plan)}
To bridge from synthetic-evidence research to production-facing insurer deployment, the following phased roadmap defines concrete milestones, evidence requirements, and progression gates.

\begin{table}[H]
\caption{Pilot roadmap with measurable gates from validation to limited deployment.}
